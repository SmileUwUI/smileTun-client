...
func (s *StreamingPacket) PackageAssembly(key, salt []byte, fake bool) (err error) {
	if s.typePacket != PlainPacket {
	...
	binary.BigEndian.PutUint16(s.rawData[3:5], uint16(len(s.cipherData)+len(nonce)+2+1))
